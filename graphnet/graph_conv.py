...
class EmbedLayerUpdate(nn.Module):
    """
    This class updates embedded layer one time
    """
            ...
            for layer_idx, layer in enumerate(layers['fixed_layers']):
                if type(layer) is nn.Conv2d:
                    ...
                    layer_n_bounds = th.cat([beta.unsqueeze(-1),
                                             ...
                                             dual_vars[relu_count_idx][:, 1].unsqueeze(-1) - dual_vars[relu_count_idx][
                                                                                             :, 2].unsqueeze(-1),
            ...
            for layer_idx, layer in reversed(list(enumerate(layers['fixed_layers']))):
                ...
                if type(layer) is nn.Conv2d:
                    ...
                    if type(layers['fixed_layers'][layer_idx - 1]) is nn.Conv2d:
                        ...
                        pre_layer_bias = pre_layer_bias.unsqueeze(1).expand(out_features[1],
                                                                            out_features[2] * out_features[3])
                        pre_layer_bias = pre_layer_bias.reshape(-1)
...
class EmbedUpdates(nn.Module):
    """
    this class updates embedding vectors from t=1 and t=T
    """
