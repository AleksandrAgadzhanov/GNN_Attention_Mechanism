...
def main():
    ...
    loss_lambdas = np.logspace(math.log10(args.start_lambda), math.log10(args.end_lambda), num=2)
    loss_lambdas = [round(loss_lambda, 6) for loss_lambda in loss_lambdas]

    for loss_lambda in loss_lambdas:
        cross_validate_gnn(loss_lambda, 'train_SAT_jade_reduced_dataset.pkl', 'val_SAT_jade.pkl', 'cifar_base_kw',
                           0.00001, 50, 0.1, 100, 1, 30, 2, log_filepath=log_filepath, device='cuda')
