...
class KWConvNetwork(LinearizedNetwork):
    ...
    def get_lower_bound(self, relu_mask, pre_lbs, pre_ubs, decision, choice):
        try:
            start = time.time()
            gub, glb, ub_point, new_mask, lower_bounds, upper_bounds = self.update_the_model(relu_mask, pre_lbs,
                                                                                             pre_ubs, decision, choice)
            end = time.time()
    ...
    def build_the_model(self, input_domain, image, ball_eps, image_is_bounded):
        """
        ...
        NOTE: we keep all bounds as a list of tensors from now on.
              ...
              Mask is linearized
              Gurobi_var lists are linearised
              self.model_lower_bounds and self.model_upper_bounds are kept mainly for
        ...
        # first get KW bounds
        self.loose_dual = LooseDualNetworkApproximation(self.layers, image, ball_eps)
        kw_lb, kw_ub, pre_relu_indices, dual_info = self.loose_dual.init_kw_bounds(image_is_bounded)
        ...
        for pre_idx in pre_relu_indices:
            if torch.sum(abs(lower_bounds[pre_idx] - kw_lb[pre_idx]) > 1e-4) == 0 and torch.sum(
                ...
                lower_bounds, upper_bounds, dual_info = self.loose_dual.update_kw_bounds(pre_idx,
                                                                                         pre_lb_all=lower_bounds,
                                                                                         pre_ub_all=upper_bounds,
                                                                                         dual_info=dual_info)
                break

        # record the dual_info as an attribute of the loose_dual instance this should be the only dual instance recorded
        ...
        for layer in self.layers:
            ...
            if type(layer) is nn.Linear:
                ...
                for neuron_idx in range(layer.weight.size(0)):
                    ...
                    v = self.model.addVar(lb=out_lb, ub=out_ub, obj=0, vtype=grb.GRB.CONTINUOUS,
                ...
                if isinstance(self.gurobi_vars[-1][0], list):
                    ...
                    for neuron_idx, pre_var in enumerate(self.gurobi_vars[-1]):
                        ...
                        if slope == 1.0:
                            ...
                            v = zero_var
                            # No need to add an additional constraint that v==0 because this will be covered by the
                            # bounds we set on the value of v.
                            new_layer_mask.append(0)
                        else:
                            lb = 0
                            ub = pre_ub
        ...
        return lower_bounds, upper_bounds, pre_relu_indices
    ...
    def update_the_model(self, relu_mask, pre_lb_all, pre_ub_all, decision, choice):
        ...
        lower_bounds, upper_bounds, new_dual_info = self.loose_dual.update_kw_bounds(self.replacing_bd_index,
                                                                                     pre_lb_all, pre_ub_all, decision,
                                                                                     choice)
        ## DEBUG
        ...
        for layer in self.layers[first_changed_bounds_index + 1:]:
            if type(layer) is nn.Linear:
                ...
                out_ubs = pos_w @ pre_ub + neg_w @ pre_lb + layer.bias
                # Get the better estimates from KW and Interval Bounds
                if torch.sum(lower_bounds[inter_bounds_index] >= out_lbs).item() != len(
                        lower_bounds[inter_bounds_index]):
                ...
                if torch.sum(upper_bounds[inter_bounds_index] <= out_ubs).item() != len(
                        upper_bounds[inter_bounds_index]):
                ...
                if torch.sum(lower_bounds[inter_bounds_index] >= out_lbs).item() != len(
                        lower_bounds[inter_bounds_index].view(-1)):
                    lower_bounds[inter_bounds_index] = torch.max(lower_bounds[inter_bounds_index], out_lbs).squeeze(0)
                    change_idx = min(inter_bounds_index, change_idx)

                if torch.sum(upper_bounds[inter_bounds_index] <= out_ubs).item() != len(
                        upper_bounds[inter_bounds_index].view(-1)):
                    upper_bounds[inter_bounds_index] = torch.min(upper_bounds[inter_bounds_index], out_ubs).squeeze(0)
                    change_idx = min(inter_bounds_index, change_idx)
        ...
        if change_idx < len(lower_bounds) - 1:
            ...
            lower_bounds, upper_bounds, _ = self.loose_dual.update_kw_bounds(self.replacing_bd_index, lower_bounds,
                                                                             upper_bounds)
            ## DEBUG
...
# )
#
# lower_bounds = torch.zeros([3, 32, 32])
# upper_bounds = torch.ones([3, 32, 32])
# input_domain = torch.zeros([3, 32, 32, 2])
# input_domain[:, :, :, 0] = lower_bounds
# input_domain[:, :, :, 1] = upper_bounds
# image = torch.rand([1, 3, 32, 32])
#
# kw_net = KWConvNetwork(list(model.children()))
# lower_bounds_all, upper_bounds_all, pre_relu_indices_all = kw_net.build_the_model(input_domain, image, 0.1, False)
#
# for i in range(len(lower_bounds_all)):
#     print(lower_bounds_all[i].size())
