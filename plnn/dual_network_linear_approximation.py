...
class LooseDualNetworkApproximation(LinearizedNetwork):
    ...
    def init_kw_debug(self, bounded, pre_lb_all=None, pre_ub_all=None, decision=None, choice=None):
        ...
        if pre_lb_all is None and pre_ub_all is None:
            ...
            dual = DualNetwork(self.net, self.x, self.ball_eps, bounded_input=bounded, provided_zl=pre_relu_lb,
                               provided_zu=pre_relu_ub)
    ...
    def update_kw_bounds(self, dual_replace_idx, pre_lb_all=None, pre_ub_all=None, decision=None, choice=None,
                         dual_info=None):
        '''
        ...
        # import pdb; pdb.set_trace()
        lower_bounds.append(
            max(self.last_layer_objective(torch.ones(1, 1, 1), new_dual_net, dual_info[1]).view(-1), pre_lb_all[-1]))
        upper_bounds.append(
            min(-self.last_layer_objective(-torch.ones(1, 1, 1), new_dual_net, dual_info[1]).view(-1), pre_ub_all[-1]))
